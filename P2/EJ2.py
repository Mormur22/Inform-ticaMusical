...
def oscChucksquare(frec,vol):
    global last # var global
    data = np.float32(vol*np.sign(np.sin(2*np.pi *(np.arange(CHUNK)+last)* frec/SRATE)))
    last += CHUNK # actualizamos ultimo generado
    return data

def oscChucktriangle(frec,vol):
    global last # var global
    data = np.float32(vol*np.abs(2*(np.arange(CHUNK)+last)*(frec/SRATE - np.floor(0.5 + frec/SRATE))))
    last += CHUNK # actualizamos ultimo generado
    return data

def oscChucksaw(frec,vol):
    global last # var global
    data = np.float32(vol*2*(np.arange(CHUNK)+last)*(frec/SRATE - np.floor(0.5 + frec/SRATE)))
    last += CHUNK # actualizamos ultimo generado
    return data
